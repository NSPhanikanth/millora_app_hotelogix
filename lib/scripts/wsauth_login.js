...
async function login(accessKey, accessSecret, hotelId, counterId, email)
{
    requestMethod = "login"
    timestamp = new Date().toISOString().split(".")[0]
    data = `{
...
path = process.argv[8] || `${timestamp}.json`
...
if(accessTokenRequired)
{
    ...
    .then((response) => {
        ...
        try{
            ...
            login(consumerKey, consumerSecret, hotelId, counterId, email)
                .then((response) => {
